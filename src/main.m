...
%cd("C:\Users\akgre\Documents\masters\NAVARCH 568\rob-530-project\")
...
%% Sample data
dt = 0.02;
[Robots, timesteps] = sampleMRCLAMdataSet(Robots, 0.02);
...
for i = start:end_idx
    ...
    for robot_num = 1:n_robots
        % Check if robot is active yet / still active
        if(Robots{robot_num}.G(indexes(robot_num)) - t > 0.05)
            continue;
        end
        % Else, process and update robot
        
        % Robot index
        i_r = indexes(robot_num);
        
        % Update command vector
        command_t = [Robots{robot_num}.O(i_r,2); Robots{robot_num}.O(i_r,3)];
...
%         end
    ...
    %         disp(wtot)
            ...
            Robots{robot_num}.Est(i_r,:) = [t poseMean(1) poseMean(2) ...
                poseMean(3)];
            ...
            disp(wtot);
            disp(filter.p.w);
            Robots{robot_num}.Est(i_r,:) = [t nan(1,3)];
            %error = square_err(Robots{robot_num}.G, Robots{robot_num}.Est);
        end    
        
        % Increment robot index
        indexes(robot_num) = indexes(robot_num) + 1;
...
plot(square_err(Robots{1}.G(1000:50000,2:4), Robots{1}.Est(1000:50000,2:4)));
